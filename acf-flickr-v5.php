	...
	*/
	...
	function render_field_settings( $field ) {
		...
		acf_render_field_setting( $field, array(
			...
			'instructions'	=> __('Do you want to be able to select photos from the photostream or use sets that have already been created on Flickr?','acf-flickr'),
			'type'			=> 'select',
  	...
  	$instructions .= ' ' . __('Alternatively, you can control your own caching folder by setting the constant <strong>FLICKR_FIELD_CACHE_DIR</strong>.', 'acf-flickr');
		...
		<div class="field_form flickr_field type_<?php echo $field['flickr_content_type']; ?>">
			...
			if ($field['flickr_content_type'] == 'sets' || $field['flickr_content_type'] == 'galleries') {
				...
				<table class="acf_input widefat acf_field_form_table">
					<tbody>
						<?php
						if (!empty($flickr_data) && isset($flickr_data->photosets->photoset)) {
